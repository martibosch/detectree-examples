...
def _get_window_transform(width, height, transform, num_tile_subdivisions):
    ...
    offsets = itertools.product(
        range(0, width, dst_width), range(0, height, dst_height)
    )
    big_window = windows.Window(col_off=0, row_off=0, width=width, height=height)
    for col_off, row_off in offsets:
        dst_window = windows.Window(
            col_off=col_off, row_off=row_off, width=dst_width, height=dst_height
...
@click.command()
@click.argument("intersecting_tiles_csv_filepath", type=click.Path(exists=True))
...
def main(
    intersecting_tiles_csv_filepath,
    tiles_dir,
    output_filepath,
    resample_factor,
    keep_raw,
    ...
    tile_filenames = pd.read_csv(
        intersecting_tiles_csv_filepath, index_col=0, header=None
    ).iloc[:, 0]
    output_tiles = []
    for tile_filename in tqdm.tqdm(tile_filenames):
        ...
        with rio.open(raw_tile_filepath) as src:
            ...
            data = src.read(
                out_shape=(src.count, interim_height, interim_width),
                resampling=Resampling.average,
            )
            t = src.transform
            interim_transform = affine.Affine(
                t.a * resample_factor, t.b, t.c, t.d, t.e * resample_factor, t.f
            )
            for i, (dst_window, dst_transform) in enumerate(
                _get_window_transform(
                    interim_width,
                    interim_height,
                    interim_transform,
                ...
                with rio.open(tile_filepath, "w", **profile) as dst:
                    for channel in range(src.count):
                        dst.write(
                            data[channel][
                                row_off : row_off + dst_window.height,
    ...
    if nominatim_query:
        ...
        if exclude_nominatim_query:
            logger.info(
                "Querying Nominatim for boundaries for `%s`", exclude_nominatim_query
            )
            exclude_geom = (
                ox.geocode_to_gdf(exclude_nominatim_query)["geometry"]
                .to_crs(crs)
                .iloc[0]
            )
        ...
        tiles_gdf = gpd.GeoDataFrame(
            ...
            geometry=list(map(bbox_geom_from_tile, output_tiles)),
            crs=crs,
        )
        output_tiles_ser = gpd.sjoin(
            tiles_gdf,
